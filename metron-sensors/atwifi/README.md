...
This leverages off of the Apache Pycapa sensor -- so the readme for how
that works is covered below.
...
AtWifi performs wifi packet scanning, filtering for mobile devices
and publishes the MAC addresses and signal strengths of devices it
finds nearby every 5 minutes (by default).  The cli allows you to
consume the data from our agent itself (off the wifi adapter) or from
the kafka topic queue (as a listener)


Installation
============

General notes on the installation of AtWifi.
...
1. Install system dependencies including the core development tools, Python libraries and header files, and wireshark libraries and header files.  On CentOS 7+, you can install these requirements with the following command.

    ```
    yum -y install tshark
    ```
...
1. Install atwifi.
    ...
    ```
    cd metron/metron-sensors/atwifi
...
Atwifi has three primary runtime modes.

* **Producer Mode**: Atwifi can capture packets from a compliant network
interface, which is producing a list of MAC addresses it finds and it's
associated signal strength.

* **Simulation Mode**: Atwifi can extract scanned packets from a
Tutella mongo event database and republish them (with updated timestamps)
which is useful if you don't have a network adapter that supports the
right mode.

In both of these previous modes, it is forwarding those packets to a
Kafka topic.  Atwifi embeds the nearby state data in the Kafka message
body.  The message key contains the timestamp indicating when the packet
was captured in microseconds from the epoch, in network byte order.

* **Consumer Mode**: AtWifi can also perform the reverse operation.  
It can consume packets from Kafka and reconstruct each nearby state
packet.
...
```
$ atwifi --help
usage: pycapa [-h] [-p] [-c] [-S mongdb_url ]
              [-k KAFKA_BROKERS] [-t KAFKA_TOPIC]
...
optional arguments:
  ...
  -S URL, --simulation URL 
                        read nearby state packets from a Tutella database
                        and publish them as with --producer 
